...
import UserStories from '../UserStoriesCompo'
import PostItem from '../PostItemCompo'
...
class Home extends Component {
  ...
  componentDidMount() {
    this.userStoriesView()
    this.fetchPosts()
  }

  userStoriesView = async () => {
    this.setState({
      apiStatus: 'INPROGRESS',
    })
        ...
        <div>
          ...
          <ul className="postCon">
            {searchedPosts.map(each => (
              <PostItem postDetails={each} key={each.postId} />
            ))}
          </ul>
        </div>
      )
  ...
  render() {
    return (
      <div>
        ...
        <div className="usrstrCon">{this.renderUserStories()}</div>
        <div className="userCon">{this.renderingView()}</div>
      </div>
    )
  }
}
